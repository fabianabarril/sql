...
SELECT
product_name || ', ' || COALESCE(NULLIF(TRIM(product_size), ''), '')|| ' (' || COALESCE(NULLIF(TRIM(product_qty_type), ''), 'unit') || ')' as products_list
FROM product;
...
SELECT market_date, total_sales, 'best_day' AS category
FROM ranked_sales
WHERE best_rank = 1

UNION

SELECT market_date, total_sales, 'worst_day' AS category
FROM ranked_sales
WHERE worst_rank = 1;
...
Show this by vendor_name and product_name, rather than using the IDs.
...
SELECT 
    vp.vendor_name,
    vp.product_name,
    SUM(5 * vp.original_price) AS total_per_product
